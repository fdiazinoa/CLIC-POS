   ...
   const renderProductCatalog = () => (
      <div className="min-h-full w-full max-w-[1800px] mx-auto p-4 md:p-6 xl:p-8">
         ...
         <div className="mb-4 flex flex-wrap items-center justify-between gap-3 border-b border-gray-100 pb-4">
            ...
            <div className="flex items-center gap-2 text-sm font-bold text-gray-500"><span>Vista:</span><button type="button" onClick={() => setProductDisplayMode('CARDS')} className={`flex h-10 items-center gap-2 rounded-xl px-3 ${productDisplayMode === 'CARDS' || !isLargeCatalogLayout ? 'bg-blue-600 text-white' : 'border border-gray-200 bg-white'}`}><Grid size={17} /> Tarjetas</button>{isLargeCatalogLayout && <button type="button" onClick={() => setProductDisplayMode('TABLE')} className={`flex h-10 items-center gap-2 rounded-xl px-3 ${productDisplayMode === 'TABLE' ? 'bg-blue-600 text-white' : 'border border-gray-200 bg-white'}`}><List size={17} /> Tabla</button>}</div>
         </div>
