...
import { FiscalRangeDGII, DocumentSeries, Transaction, NCFType } from '../types';
...
const DocumentSettings: React.FC<DocumentSettingsProps> = ({ onClose }) => {
  ...
  const [seriesList, setSeriesList] = useState<DocumentSeries[]>(() => {
    ...
    return terminals[0]?.config?.documentSeries || [];
  });
  
  const [editingSeries, setEditingSeries] = useState<DocumentSeries | null>(null);

  // Data for Fiscal Pool & Transactions for Audit
  const [fiscalRanges, setFiscalRanges] = useState<FiscalRangeDGII[]>(() => db.get('fiscalRanges') || []);
  
  const transactions = useMemo(() => (db.get('transactions') as Transaction[]) || [], []);
