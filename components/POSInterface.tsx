...
import { 
  ...
  Customer, Product, CartItem, Transaction, CurrencyConfig 
} from '../types';
...
const POSInterface: React.FC<POSInterfaceProps> = ({
  ...
  const handleProductClick = (product: Product) => {
    if (product.type === 'SERVICE' && product.name.toLowerCase().includes('peso')) {
  ...
  const addToCart = (product: Product, quantity: number = 1, priceOverride?: number, modifiers?: string[]) => {
    onUpdateCart(prev => {
      const existing = prev.find(i => i.id === product.id && JSON.stringify(i.modifiers) === JSON.stringify(modifiers));
      if (existing) {
  ...
  const handleResumeTicket = (parked: any) => {
    if (cart.length > 0 && !confirm("¿Reemplazar el ticket actual con el suspendido? El actual se perderá.")) return;
    ...
    <div className="flex h-screen bg-gray-100 overflow-hidden font-sans text-gray-900 relative">
      ...
      <div className={`flex-1 flex flex-col min-w-0 bg-gray-50 transition-all duration-300 ${mobileView === 'TICKET' ? 'hidden md:flex' : 'flex'}`}>
        <header className="bg-white px-8 py-4 border-b border-gray-200 flex items-center gap-6 shadow-sm z-10">
          <div className="flex-1 relative group flex items-center gap-3">
           ...
           <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-4 xl:grid-cols-5 2xl:grid-cols-6 gap-6 pb-32">
              {filteredProducts.map(product => (
                 <div 
                    ...
                    className="bg-white rounded-[2rem] p-4 shadow-sm border border-gray-100 cursor-pointer hover:shadow-xl hover:border-blue-200 hover:-translate-y-1 transition-all active:scale-95 group flex flex-col h-full relative overflow-hidden"
                 >
                    ...
                    <div className="flex flex-col flex-1">
                       <span className="text-[9px] font-bold text-blue-500 uppercase mb-1 opacity-60">{product.category}</span>
                       <h3 className="font-bold text-gray-800 text-sm leading-tight mb-2 line-clamp-2 flex-1">{product.name}</h3>
                       <div className="flex justify-between items-center mt-auto pt-2 border-t border-gray-50">
                          <span className="font-black text-lg text-gray-900">{baseCurrency.symbol}{product.price.toFixed(2)}</span>
                       </div>
                    </div>
                 </div>
              ))}
        ...
        {/* FLOATING CART BAR FOR MOBILE/COMPACT VIEW */}
        {cart.length > 0 && mobileView === 'PRODUCTS' && (
           <div className="md:hidden fixed bottom-6 left-6 right-6 z-[45] animate-in slide-in-from-bottom-10 fade-in duration-500">
              <button 
                ...
                className="w-full bg-blue-600 text-white rounded-2xl p-4 flex items-center justify-between shadow-2xl shadow-blue-500/40 ring-4 ring-white/50 active:scale-95 transition-all"
              >
      ...
      {/* 3. PANEL DERECHO: CARRITO / TICKET */}
         ...
         <div className="p-5 border-b border-gray-100 bg-gray-50/50 flex flex-col gap-3 shrink-0">
            <div className="flex justify-between items-center">
               <div className="flex items-center gap-2">
                  <button onClick={() => setMobileView('PRODUCTS')} className="md:hidden p-2 -ml-2 text-gray-400 hover:text-blue-600">
                    <ArrowLeft size={20} />
                  </button>
               ...
               <div className="flex items-center gap-1">
                  <button onClick={handleParkTicket} title="Guardar Ticket" className="p-2 text-gray-400 hover:text-blue-600 hover:bg-blue-50 rounded-lg transition-all"><Save size={18} /></button>
                  <button onClick={() => setShowParkedList(true)} title="Recuperar Ticket" className="p-2 text-gray-400 hover:text-orange-600 hover:bg-orange-50 rounded-lg transition-all relative">
                     <PauseCircle size={18} />
                  ...
                  <button onClick={onOpenHistory} title="Historial de Ventas" className="p-2 text-gray-400 hover:text-indigo-600 hover:bg-indigo-50 rounded-lg transition-all"><History size={18} /></button>
                  ...
                  <button onClick={onOpenSettings} title="Ajustes de Sistema" className="p-2 text-gray-400 hover:text-slate-900 hover:bg-slate-100 rounded-lg transition-all"><Settings size={18} /></button>
                  <div className="w-px h-4 bg-gray-200 mx-1"></div>
                  <button onClick={() => setShowTicketOptions(true)} title="Más opciones" className="p-2 text-gray-400 hover:text-blue-600 rounded-lg transition-all"><MoreVertical size={20} /></button>
               </div>
            </div>
            ...
            >
               <div className={`w-10 h-10 rounded-xl flex items-center justify-center ${selectedCustomer ? 'bg-blue-600 text-white shadow-lg' : 'bg-gray-100 text-gray-400'}`}>
                  <UserPlus size={20} />
               </div>
               <div className="text-left flex-1 truncate">
                  <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest leading-none mb-1">Cliente</p>
                  <p className="font-bold text-sm text-gray-800 truncate">{selectedCustomer?.name || 'Cliente de Contado'}</p>
               </div>
         ...
         <div className="flex-1 overflow-y-auto px-5 py-4 space-y-3 no-scrollbar">
            {cart.length === 0 ? (
               <div className="h-full flex flex-col items-center justify-center opacity-30 gap-4">
                  ...
                  <p className="font-bold text-gray-500 text-sm uppercase tracking-widest">Carrito Vacío</p>
               </div>
            ) : (
               cart.map((item) => (
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-2 mt-1">
                            <div className={`px-1.5 py-0.5 rounded text-[10px] font-black border ${item.type === 'SERVICE' && item.name.toLowerCase().includes('peso') ? 'bg-orange-50 text-orange-700 border-orange-100' : 'bg-blue-50 text-blue-700 border-blue-50'}`}>
                                {item.type === 'SERVICE' && item.name.toLowerCase().includes('peso') ? `${item.quantity.toFixed(3)} lb` : `x${item.quantity}`}
                            </div>
         ...
         <div className="p-6 bg-white border-t border-gray-100 shadow-[0_-10px_40px_rgba(0,0,0,0.05)] shrink-0">
            <div className="space-y-2 mb-4">
               ...
               <div className="flex justify-between items-start pt-2 mt-2 border-t border-gray-100">
                  <div className="flex flex-col">
                     <span className="font-bold text-gray-800 uppercase tracking-widest text-xs">Total a Pagar</span>
            ...
            <div className="grid grid-cols-2 gap-3 mb-3">
               <button 
                  ...
                  className="flex flex-col items-center justify-center py-2.5 bg-gray-100 text-gray-600 rounded-xl font-bold text-[10px] hover:bg-gray-200 transition-all disabled:opacity-50"
               >
                  <Percent size={18} className="mb-0.5" />
                  Desc.
               </button>
               <button 
                  onClick={onLogout}
                  className="flex flex-col items-center justify-center py-2.5 bg-gray-100 text-gray-600 rounded-xl font-bold text-[10px] hover:bg-red-50 hover:text-red-600 transition-all"
               >
            ...
            <button 
               ...
               disabled={cart.length === 0}
               className="w-full py-4 bg-blue-600 text-white rounded-2xl font-black text-lg shadow-xl shadow-blue-500/30 hover:bg-blue-700 active:scale-95 transition-all disabled:opacity-50 flex items-center justify-center gap-3"
            >
               <CreditCard size={24} />
               PAGAR
            </button>
                     ...
                     parkedTickets.map(pt => (
                        <div key={pt.id} className="bg-white p-5 rounded-2xl border border-gray-200 flex justify-between items-center group hover:border-orange-300 transition-all">
                           <div className="flex-1">
                              ...
                              <p className="text-sm text-gray-500">{pt.customer?.name || 'Cliente de Contado'} • {pt.items.length} items</p>
      ...
      {showTicketOptions && (
         <TicketOptionsModal 
            ...
            onAction={(id) => {
               ...
               if (id === 'PARK_SALE') handleParkTicket();
               if (id === 'ASSIGN_CUSTOMER') onOpenCustomers();
      ...
      {editingItem && (
         <CartItemOptionsModal 
            item={editingItem} config={config} users={[]} 
            onClose={() => setEditingItem(null)} onUpdate={updateCartItem}
            canApplyDiscount={true} canVoidItem={true}
         />
      )}

      {selectedProductForVariants && (
         <ProductVariantSelector 
            product={selectedProductForVariants} currencySymbol={baseCurrency.symbol}
            onClose={() => setSelectedProductForVariants(null)}
            onConfirm={(p, mods, price) => { addToCart(p, 1, price, mods); setSelectedProductForVariants(null); }}
         />
      )}

      {productForScale && (
         <ScaleModal 
            product={productForScale} currencySymbol={baseCurrency.symbol}
            onClose={() => setProductForScale(null)}
            onConfirm={(weight) => { addToCart(productForScale, weight); setProductForScale(null); }}
         />
      )}
