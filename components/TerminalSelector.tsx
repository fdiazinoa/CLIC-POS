...
const getSetupApiBase = (masterIp?: string): string => {
  ...
  if (!normalized) {
    return `${window.location.protocol}//${window.location.hostname}:3001/api/setup`;
  }
  ...
  const bindTerminal = useCallback(
    ...
    try {
      ...
      if (isNativeAndroid && erpBaseUrl) {
        ...
        if (isNativeAndroid && erpBaseUrl) {
          try {
            const erpInitialConfigResponse = await fetch(
              `${erpBaseUrl}/api/setup/initial-config/${encodeURIComponent(data.erp_terminal_id || terminal.erpTerminalId || data.terminal_id || terminal.id)}?${initialConfigParams.toString()}`
            );

            if (erpInitialConfigResponse.ok) {
              const erpInitialConfigData = (await erpInitialConfigResponse.json()) as InitialConfigResponse;
              ...
              } else if (erpInitialConfigData?.config) {
                initialConfigData = {
                  ...erpInitialConfigData,
                  terminal_id: data.terminal_id || terminal.id,
                  erp_terminal_id: data.erp_terminal_id || terminal.erpTerminalId || data.terminal_id || terminal.id,
              ...
              console.warn(`⚠️ ERP initial-config returned ${erpInitialConfigResponse.status}; continuing with bind config fallback.`);
            }
          } catch (initialConfigError) {
            console.warn('⚠️ Native ERP initial-config fetch failed; continuing with bind config fallback.', initialConfigError);
          }
        } else {
