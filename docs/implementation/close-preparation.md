...
La imagen queda dentro del mismo documento atómico que el reporte, sus miembros y declaración. Reabrir SQLite conserva los bytes; un cambio posterior de config invalida resume pero no sustituye la imagen anterior. El productor ERP requiere compatibilidad explícita para consumir este subconjunto verificando el artifact congelado y ambos hashes. La ruta legacy de laboratorio que recibe la configuración completa sigue siendo diferente; no debe presentarse como prueba de consumo del artifact nuevo.

Pruebas: cuatro salidas completas del corpus padre reproducidas con la proyección, precedencia de métodos duplicados, presencia/undefined/null, exclusión de credenciales y persistencia en SQLite. No se modifican vectores históricos ni se habilita cierre. La prueba cruzada de esta entrega todavía usa la ruta legacy de configuración completa de ERP 52f533bb; la integración del artifact proyectado se realizará cuando ERP entregue su compatibilidad.
