...
const expression=`(async()=>{
 ...
 for(let pair=0;pair<6;pair++)for(const enabled of (pair%2?[true,false]:[false,true])){d.disable();await new Promise(r=>setTimeout(r,150));if(enabled){d.enable?.();d.arm(20);}const start=performance.now();const checksum=await d.run('calibration:continuations',work);results.push({pair,enabled,ms:performance.now()-start,checksum});}
