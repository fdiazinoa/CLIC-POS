...
const expression=`(async()=>{
 ...
 const work=async()=>{let checksum=0;for(let k=0;k<8;k++){await Promise.resolve();checksum+=d.processing('calibration:parse-map-stringify',()=>JSON.parse(JSON.stringify(rows)).filter(x=>x.q>3).map(x=>x.id+x.q).reduce((a,b)=>a+b,0));}return checksum;};
